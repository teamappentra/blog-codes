...
* image. Upon failure, this function returns 0, upon sucess it returns 1.
******************************************************************************/
int read_y_image(char *infilename, unsigned char **image, int *rows,
    int *cols)
{
    FILE *fp;
    char buf[71];
    ...
    ***************************************************************************/
    ...
    ret_i = fread((*image), (*cols), (*rows), fp);
    if (ret_i != *rows) {
        fprintf(stderr, "Error reading the image data in read_pgm_image().\n");
        if (fp != stdin) fclose(fp);
        free((*image));
...
* outfilename = NULL. A comment can be written to the header if coment != NULL.
******************************************************************************/
int write_y_image(char *outfilename, unsigned char *image, int rows,
    int cols, char *comment, int maxval)
{
    FILE *fp;
    ...
    float tlow, float thigh, unsigned char *edge)
{
    ...
    ****************************************************************************/
    ...
    #pragma omp parallel for private(r,c, pos) shared(edge,mag,rows,cols) reduction(+:hist[:32768]) default(none)
    for (r = 0; r<rows; r++){
        pos = r * cols;
        for (c = 0; c<cols; c++, pos++){
...
void non_max_supp(short* mag,
     ...
     ****************************************************************************/
    for (firstindex = 0, lastindex = ncols * (nrows - 1); firstindex < ncols;
         firstindex++, lastindex++) {
        result[firstindex] = 0;
        result[lastindex] = 0;
    }

    for (firstindex = 0, lastindex = ncols - 1; firstindex < nrows;
         firstindex += ncols, lastindex += ncols) {
        result[firstindex] = 0;
        result[lastindex] = 0;
    }
    ...
    for (rowcount = 1; rowcount < nrows - 2; rowcount++) {
        ...
        for (colcount = 1; colcount < ncols - 2; colcount++) {
            ...
            if (gx >= 0) {
                if (gy >= 0) {
                    if (gx >= gy) {
                        /* 111 */
                        /* Left point */
                        z1 = mag[index - 1];//*(magptr - 1);
                        z2 = mag[index - ncols - 1];//*(magptr - ncols - 1);
                        ...
                        /* 110 */
                        /* Left point */
                        z1 = mag[index - ncols]; //*(magptr - ncols);
                        z2 = mag[index - ncols - 1];//*(magptr - ncols - 1);
            ...
            if ((mag1 > 0.0) || (mag2 > 0.0)) {
                result[index] = (unsigned char)NOEDGE;
            } else {
                if (mag2 == 0.0)
                    result[index] = (unsigned char)NOEDGE;
                else
                    result[index] = (unsigned char)POSSIBLE_EDGE;
            }
        }
    }
